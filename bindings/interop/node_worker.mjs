...
const verifyDelegationMultihop =
  mod.verifyDelegationMultihop ??
  mod.default?.verifyDelegationMultihop;
...
const methods = {
  ...
  verify_delegation: (p) => {
    // Strict v0.2 by default; route to the multi-hop verifier only when the
    // caller explicitly requests a multi-hop ceiling (draft RFC-AITP-0011).
    const hops = p.max_hops ?? 0;
    const v =
      hops > 0
        ? verifyDelegationMultihop(p.token, p.verifier_aid, hops)
