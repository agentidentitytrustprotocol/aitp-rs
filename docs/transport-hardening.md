...
| `net_guard.rs` | SSRF `HostGuard` on peer-derived fetches (Manifest, JWKS/discovery/`aitp-keys`, facade handshake/renew): redirect-block, resolved-address classification (link-local/metadata always denied; private ranges warn), DNS-rebind-safe address pinning | 0009 | done | Default `HostGuard::WarnPrivate`; internet-facing deployments call `.with_host_guard(HostGuard::strict())`. `DenyPrivate`/`AllowAll` also available. |
